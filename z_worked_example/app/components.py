...
def get_all_stocks():
    ...
    with open('db/stock_db.json','r') as json_file:
...
def get_stock_by_ticker(ticker_symbol):
    ...
    stock_filter = list(filter(lambda x:x.ticker_symbol == ticker_symbol, stock_list_obj))
    if len(stock_filter) == 0:
        return None
    
    return stock_filter[0]

def save_stock(stock_to_save):

    if get_stock_by_ticker(stock_to_save.ticker_symbol) == None:
        stock_list_obj = get_all_stocks()
        stock_list_obj.append(stock_to_save)
        ...
        raise Exception(f"{stock_to_save.ticker_symbol} already exists")
